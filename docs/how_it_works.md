...
본 문서는 `generate-tizen-app` 프로젝트에서 대형 언어 모델(LLM) 기반의 에이전트가 어떻게 자연어 요구사항을 실제 Tizen C#앱으로 완벽하게 컴파일되는 코드로 변환해 내는지에 대한 원리를 설명합니다.
...
- **효과**: 카탈로그에 모든 Tizen 시스템 API를 담지 않아도(모두 담을 경우 문맥 길이가 지나치게 늘어나 비용/속도/정확도 저하 발생), **UI 외의 기능들은 LLM의 강력한 배경 지식이 자동으로 보완합니다.**
...
하지만, LLM의 내장 지식(Tizen.FX 등)을 100% 신뢰할 수는 없습니다. 플랫폼 버전이 업데이트되어 이전 메서드(`Start()` -> `Play()`)가 Deprecated 되거나 파라미터가 변경되었을 수 있습니다. 이를 극복하기 위해 **Self-Healing 루프**를 활용합니다.
...
4. LLM이 이전 버전의 지식을 바탕으로 오류를 일으킨 부분을 인지하고, 올바른 방식(또는 Microsoft Learn MCP를 활용한 최신 공식 문서 실시간 검색)으로 코드를 **스스로 수정 및 재빌드**합니다. (최대 3회 반복)
