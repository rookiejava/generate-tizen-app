...
{
    public class SkiaViewTest2 : Grid, ITestCase
    {
        ...
        string[] iconSvgs = new string[] {
            PathAdd, PathApp, PathArrowBack, PathArrowDownward, PathArrowForward, PathArrowUpward, PathCheck, PathClose, PathExpandLess, PathExpandMore, PathLink, PathMenu, PathMoreHoriz, PathMoreVert,
            PathRemove, PathRemoveCircle, PathSearch, PathReplay, PathAccount, PathSettings, PathArrowLeft, PathArrowRight };

        int cur = 0;
        public SkiaViewTest2()
        {
            ...
            canvas.PaintSurface += (s, e) =>
            {
                ...
                })
                {
                    for (int i = 0; i < 5; i++)
                    {
                        for (int j = 0; j < 5; j++)
                        {
                            using var path = SKPath.ParseSvgPathData(iconSvgs[cur]);
                            canvas.DrawPath(path, paint);
                            canvas.Translate(new SKPoint(20, 0));
                            Next();
                        }
                        canvas.Translate(new SKPoint(-100, 20));
                    }
