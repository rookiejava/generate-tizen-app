...
{
    class ScrollByFocusTest1 : Grid, ITestCase
    {
        ...
        public ScrollByFocusTest1()
        {
            ...
            for (int row = 0; row < 50; row++)
            {
                ...
                {
                    grid.Children.Add(new Button($"{row * 50 + col}")
                    {
                        Focusable = true,
            ...
            FocusManager.Instance.FocusChanged += Instance_FocusChanged;
        ...
        void Instance_FocusChanged(object sender, FocusChangedEventArgs e)
        {
            if (e.FocusedView is Button btn)
            {
                (this["Log"] as TextView).Text = $"Focused : {btn.Name}";
            }
        }
